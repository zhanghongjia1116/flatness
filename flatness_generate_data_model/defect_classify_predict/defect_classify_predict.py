...
"""
Module implementing MainWindowC.
"""
import os
...
from flatness_generate_data_model.defect_classify_predict.Ui_defect_classify_predict import Ui_MainWindowC
from flatness_generate_data_model.defect_classify_predict.myplot import MyFigure


class MainWindowC(QMainWindow, Ui_MainWindowC):
    """
    ...
    def __init__(self, parent=None):
        ...
        """
        super(MainWindowC, self).__init__(parent)
        self.setupUi(self)
    ...
    def on_help_triggered(self):
        from flatness_generate_data_model.defect_classify_predict.help import MainWindow
    ...
    def on_pushButton_2_clicked(self):
        ...
        #         test_true = []
        #         for i in range(5):
        #             model = joblib.load('%sDeg%s.m' % self.comboBox.currentText(), i)
        #
        #             ytest_true = self.data.loc[:, 'ydeg%s' % i]
        #             test_true.append(ytest_true)
        #             ytest_pre = model.predict(self.xtest)
        #             test_pre.append(ytest_pre)
        #         self.test_pre = pd.DataFrame(test_pre).T
        #         self.test_true = pd.DataFrame(test_true).T
        #
        #         label = []
        #         flat_error = self.data['yF5 flatness error'].tolist()
        #         deg = np.array(self.test_pre)
        #         lambdas = np.array([0, 0.5063, 0.3947, 0.3336, 0.2968])
        #         for i in range(self.data.shape[0]):
        #             del_iu2 = deg[i] * lambdas
        #             del_iu = abs(del_iu2)
        #             if flat_error[i] > 2.5:
        #
        #                 pot = del_iu.tolist().index(max(del_iu))
        #                 if del_iu2[pot] >= 0:
        #                     label.append(pot)
        #                 else:
        #                     label.append(-pot)
    ...
    def on_pushButton_3_clicked(self):
        ...
        try:
            if self.comboBox_2.currentText() != '请选择想要评估的钢卷号':
                acc = [accuracy(self.data[one].loc[:, 'y多分类'], self.C_pre[one]) for one in range(len(self.data))]
    ...
    def on_pushButton_7_clicked(self):
        ...
        try:
            ...
            pass
        try:
            self.C_plot(self.data[self.juan_hao_index].loc[:, 'y多分类'], self.C_pre[self.juan_hao_index])
        except:
    ...
    def on_pushButton_4_clicked(self):
        ...
        """
        try:
            # save_path = QFileDialog.getSaveFileName(self, "预测结果保存", os.getcwd(), "CSV Files(*.csv);;Excel Files(*.xlsx)")[0]
    ...
    def C_plot(self, true, pre):
        ...
        axes.set_title('%s多分类混淆矩阵' % self.juan_hao[self.juan_hao_index])
