...
def zhuzhuangtu(fengeshu,data):
    ...
    for i in range(0,fengeshu):
        # print(min(dataSignal) + deleta * i)
        # print(max(dataSignal)-deleta*(fengeshu-i-1))
        for j in data:
            if i!=fengeshu-1:
...
def mat_read(path):
    ...
    steel_en_thick = ''.join([chr(i) for i in mat[ganngjuanhao]['fileinfo']['Technostring_1entry_thickness']][:-2]).replace(',','.')
    steel_out_thick = ''.join([chr(i) for i in mat[ganngjuanhao]['fileinfo']['Technostring_1exit_thickness']][:-2]).replace(',','.')
    ...
    for big_name in zuming:
        ...
        for small_name in bianliangming:
            ...
            data = group_3['dataSignal'][0]  # 变量数据
            lenth = len(data)
            ...
            if pot == 0:
                ...
                #     result.insert(0,small_name,dataSignal[:len(result)])
                result[small_name] = data[4:4+len(result)]
                # else:
                #     result=pd.concat([result,pd.DataFrame(dataSignal[:len(result)],columns=[small_name])],axis=1)
                pot += 1
    # return  result
    return {'dataSignal':result,'Cold_Num':ganngjuanhao,'steel_grade':steel_grade,'steel_en_thick':steel_en_thick,
...
def free_chazhi(row):
    ...
    import numpy as np
    new_row=np.zeros(80)
    pot_62=np.array([13,39,65,91,117,143,169,195,221,247,273,299,325,351,377,403,429,455,481,507,533,559,585,611,637,
            676,728,780,832,884,936,988,1040,1092,1141,1196,1248,
            1287,1313,1339,1365,1391,1417,1443,1469,1495,1521,1547,1573,1599,1625,1651,1677,1703,1729,1755,1781,1807,1833,1859,1885,1911])
    num=len(row)
    long=12
    short=num-long
    ...
    for i in range(80):
        if i==0:
            ...
            new_row[i]=row[-1]+(row[-1]-row[-2])*(after_pot[-1]-before_pot[-1])/(before_pot[-1]-before_pot[-2])
        else:
            if after_pot[i]>before_pot[k]:
                k += 1
                new_row[i]=((row[k]-row[k-1])*(after_pot[i]-before_pot[k-1]))/(before_pot[k]-before_pot[k-1])+row[k-1]
...
def drop_dig(data):
    ...
    for name in names:
        # print(set(dataSignal[name].values))
...
def make_df_todether(dir_path):
    ...
    for i in range(len(pathes)):
        ...
        # dataSignal.drop(columns=['RCH_3-RCH3_Wedge_adj_OS_actual_pos'])
        # dataSignal=dataSignal[dataSignal['FLC5-F5_strip_length']>=50]
        if i==0:
            result=data
        else:
            frames=[result ,data]
