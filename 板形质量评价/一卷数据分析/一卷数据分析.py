...
from ..utils import Wlearn, fig
from ..utils.Wlearn import jugde_row_col, free_chazhi, lerangde_fenjie, search, base_file_name
from ..utils.绘图 import iu_error, get_rms, get_cha, odd_lerangde_nihe, even_lerangde_nihe, qiujiaodu
from .Ui_一卷数据分析 import Ui_MainWindow
from ..utils.flatpingjia import iu_percent_fendaitou, IU_zhibiao_fendaitou, shuju, shuju_redutu, shuju_langxin, \
...
class FullLengthQuality(QMainWindow, Ui_MainWindow):
    ...
    def on_pushButton_4_clicked(self):
        ...
        if self.lineEdit.text() != '':
            ...
            self.cwd = os.getcwd()  # 获取当前程序文件位置
            print(self.cwd.replace('\\', '/'))
            path = resource_path(self.cwd.replace('\\', '/') + '板形综合评价报表/一卷数据分析/HB19B14703700_1flat.csv')
            self.lineEdit.setText(path)
        # print(path)
