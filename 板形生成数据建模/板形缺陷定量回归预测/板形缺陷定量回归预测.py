...
class mainWindow(QMainWindow, Ui_mainWindow):
    ...
    def on_comboBox_activated(self, p0):
        ...
        try:
            ...
            if p0 == 'BP':
                self.scalarX = joblib.load(resource_path('板形生成数据建模/板形缺陷定量回归预测\\定量回归模型\标准化X.m'))
                self.scalarY = joblib.load(resource_path('板形生成数据建模/板形缺陷定量回归预测\\定量回归模型\标准化Y.m'))
    ...
    def on_pushButton_clicked(self):
        ...
        try:
            ...
            if len(self.data_path) == 0:
                ...
                self.data = [pd.read_csv(file, encoding='gbk').dropna().reset_index(drop=True) for file in self.data_path]
        ...
        try:
            ...
            if self.comboBox.currentText() == 'CatBoost':
                ...
                for j in range(len(self.data)):
                    ...
                    test_true = []
                    for i in range(5):
                        model = joblib.load(resource_path('flatness_regression\\定量回归模型\%sDeg%s.m' % (self.comboBox.currentText(), i)))
                ...
                model = load_model(resource_path('板形生成数据建模/板形缺陷定量回归预测\\定量回归模型\%s.h5'%self.comboBox.currentText()))
        ...
        try:
            if self.comboBox_3.currentText() != '请选择想要评估的钢卷号':
                if self.comboBox_2.currentText() != '选择系数':
                    ...
                    r2 = [r2_score(self.test_true[yang_ben].iloc[:, i], self.test_pre[yang_ben].iloc[:, i]) for yang_ben in range(len(self.test_true))]
                    mse = [mean_squared_error(self.test_true[yang_ben].iloc[:, i], self.test_pre[yang_ben].iloc[:, i]) for yang_ben in range(len(self.test_true))]
                ...
                QMessageBox.information(self, '提示框', '您未选择想要评估的钢卷号，请在上一个下拉框选择相应钢卷号', QMessageBox.Ok)
        except:
    ...
    def on_pushButton_6_clicked(self):
        ...
        try:
            ...
            self.test_pre[self.juan_hao_index].iloc[:, :].to_csv('%s\\%s.csv' % (save_path, self.juan_hao[self.juan_hao_index]), encoding='gbk', index=False)
    ...
    def on_pushButton_4_clicked(self):
        ...
        try:
            ...
            for i in range(len(self.juan_hao)):
                self.test_pre[i].iloc[:, :].to_csv('%s\\%s.csv' % (save_path, self.juan_hao[i]), encoding='gbk', index=False)
            self.textEdit.setText('文件已保存完毕')
        except:
            QMessageBox.information(self, '提示框', '保存出错', QMessageBox.Ok)
